...
if __name__ == '__main__':
    ...
    for i in range(len(leaf)):
        ...
        # fit the SVDD model
        exec(f"svdd{i} = BaseSVDD(C=0.6, gamma=10, kernel='rbf', display='on',degree=7)")
        # svdd = BaseSVDD(C=0.6, gamma=50, kernel='rbf', display='on',degree=4)
        # 剔除标签那一列
        SubDataArr_p = np.delete(SubDataArr_p, np.shape(SubDataArr_p)[1] - 1, 1)
        lable_1 = []
        for k in range(len(SubDataArr_p)):
            lb_1 = 1
            lable_1.append(lb_1)
        lable_1 = np.array(np.mat(lable_1).T)

        kpca1 = KernelPCA(n_components=2, kernel="rbf", gamma=30, fit_inverse_transform=True)
        SubDataArr_p = kpca1.fit_transform(SubDataArr_p)

        # 训练svdd
        eval(f'svdd{i}').fit(np.array(SubDataArr_p), lable_1)

        # plot the distance curve
        radius = eval(f'svdd{i}').radius
        distance = eval(f'svdd{i}').get_distance(SubDataArr_p)
        eval(f'svdd{i}').plot_distance(radius, distance)
        # plot the boundary
        eval(f'svdd{i}').plot_boundary(SubDataArr_p, lable_1)
        svdd_centers.append(eval(f'svdd{i}').center)
        print(eval(f'svdd{i}').center)
        # print(eval(f'svdd{i}').support_vectors)
        ...
        prepare_list['subspace_matrix' + str(i)] = np.delete(prepare_list['subspace_matrix' + str(i)],
                                                             np.shape(prepare_list['subspace_matrix' + str(i)])[1] - 1, 1)
        # 降维
        prepare_list['subspace_matrix' + str(i)] = kpca1.fit_transform(prepare_list['subspace_matrix' + str(i)])

        predictions = eval(f'svdd{i}').predict(np.array(prepare_list['subspace_matrix' + str(i)]),np.array(y) )

        # 计算样本密度
        sub_x = prepare_list['subspace_matrix' + str(i)][:,0]
        sub_y = prepare_list['subspace_matrix' + str(i)][:,1]
        xy = np.vstack([sub_x, sub_y])
        ...
        b = np.lexsort(-z.T)
        z =  z[b[0],:]
        sub_boss.append(z[0,0])


        # errArr = np.mat(np.ones((len(prepare_list['subspace_matrix' + str(i)]), 1)))
        # print("训练集子空间样本个数：", len(prepare_list['subspace_matrix' + str(i)]))
        # print("正例个数：", p)
        # print("负例个数：", n)
        # print("TP: ", errArr[(predictions == 1) & (predictions == np.mat(y))].sum())
        # print("FN: ", errArr[(predictions == -1) & (predictions != np.mat(y))].sum())
        # print("TN: ", errArr[(predictions == -1) & (predictions == np.mat(y))].sum())
        # print("FP: ", errArr[(predictions == 1) & (predictions != np.mat(y))].sum())
        # FN = errArr[(predictions == -1) & (predictions != np.mat(y))].sum()
        # FP = errArr[(predictions == 1) & (predictions != np.mat(y))].sum()
        # P = np.sum(np.mat(y).T == 1)
        # N = np.sum(np.mat(y).T == -1)
        # print("TC:", FN * (N / P) + FP)
        # print('测试集的错误率:%.3f%%' % float(errArr[predictions != np.mat(y)].sum() / len(prepare_list['subspace_matrix' + str(i)]) * 100))
        # print('测试集的正确率:%.3f%%' % float(errArr[predictions == np.mat(y)].sum() / len(prepare_list['subspace_matrix' + str(i)]) * 100))
        # Specificity_Test = errArr[(predictions == -1) & (np.mat(y) == -1)].sum() / (
        #         errArr[(predictions == -1) & (np.mat(y) == -1)].sum() + FP)
        # print("accuracy: ", accuracy_score(np.mat(y), predictions))
        # print("Specificity_Test: ", Specificity_Test)
        # print("recall: ", recall_score(np.mat(y), predictions))
        # print("precision: ", precision_score(np.mat(y), predictions))
        # print("f1_score: ", f1_score(np.mat(y), predictions))
        # print("f2_score: ", (5 * precision_score(np.mat(y), predictions) * recall_score(np.mat(y), predictions)) /
        #       (4 * precision_score(np.mat(y), predictions) + recall_score(np.mat(y), predictions)))
        # print("AUC: ", roc_auc_score(np.mat(y), predictions))
        # print("G-mean", math.sqrt(recall_score(np.mat(y), predictions) * Specificity_Test))

        print("---------------------------------------")

    sub_boss = np.mat(sub_boss).T

    kpca2 = KernelPCA(n_components=2, kernel="rbf", gamma=30, fit_inverse_transform=True)
    testArr = kpca2.fit_transform(testArr)
        # 对于每一个样本：

    svdd_centers =np.mat(np.array(svdd_centers))

    predicts = []
    for k in range(np.shape(testArr)[0]):
        dis = []
        # 计算到各个空间的距离
        for j in range(np.shape(svdd_centers)[0]):
            # dis_centers = distEclud2(np.mat(testArr)[k,:],svdd_centers[j,:],np.shape(svdd_centers)[0])
            dis_centers = embedding_distance(np.mat(testArr)[k,:],svdd_centers[j,:])
            # dis_centers = embedding_distance(np.mat(testArr)[k, :], sub_boss[j, :])
            dis.append(dis_centers)
        dis = np.mat(dis).T
        # 子空间序号矩阵
        subspace_center_index = []
        for a in range(np.shape(svdd_centers)[0]):
