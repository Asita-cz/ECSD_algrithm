...
if __name__ == '__main__':
    ...
    if ((np.shape(dataArr_N1)[0])/(np.shape(dataArr_P1)[0]))<= 255:

        # 训练集数据变换
        lmnn = LargeMarginNearestNeighbor(n_neighbors=1, verbose=2, random_state=0)
    ...
    renn = RepeatedEditedNearestNeighbours(sampling_strategy = 'auto',n_neighbors=1,n_jobs =2)
    ...
    data_processing_time = data_processing_end_time-start_time

    at1 = annoytree(dataArr,200)

    Kannoy_end_time = time.time()
    Kannoy_time = Kannoy_end_time - data_processing_end_time
    ...
    subspace_data_augmentation_end_time = time.time()
    subspace_data_augmentation_time = subspace_data_augmentation_end_time-Kannoy_end_time
    ...
    for k in range(np.shape(testArr)[0]):
        ...
        for re in range(int(np.shape(sub_density_center)[0]/divided_num)):
            ...
            if ((np.shape(dataArr_N1)[0]) / (np.shape(dataArr_P1)[0])) <= 30:
                ...
                # predict = (dis_a[re, 1] / all) * eval(f'MLP{int(dis_a[re, 0])}').predict(np.array(np.mat(testArr[k, :])))
            else:
                # predict = (dis_a[re, 1]/all)*eval(f'adboost_{int(dis_a[re, 0])}').predict(np.array(np.mat(testArr[k, :])))
                predict = (dis_a[re, 1] / all) * eval(f'GBDT{int(dis_a[re, 0])}').predict(np.array(np.mat(testArr[k, :])))
            # print(predict)
    ...
    Classifier_ensemble_end_time = time.time()
    Classifier_ensemble_time = Classifier_ensemble_end_time-subspace_data_augmentation_end_time
    ...
    print("time:\n")
    print("data_processing_time",data_processing_time)
    print("Kannoy_time", Kannoy_time)
    print("subspace_data_augmentation_time", subspace_data_augmentation_time)
    print("Classifier_ensemble_time", Classifier_ensemble_time)
