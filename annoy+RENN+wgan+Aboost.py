...
if __name__ == '__main__':
    ...
    # #数据变换
    # lmnn = LargeMarginNearestNeighbor(n_neighbors=3, verbose=1, random_state=0)
    ...
    for i in range(len(leaf)):
        ...
        # # 数据筛选后的训练集散点图:
        # showDataSet1(i,fake_data, dataArr_N)

        # train_x, train_y = makeNewTrdata(fake_data, dataArr_N)
        ...
        # train_x = lmnn.transform(train_x)
        # train_xy = np.concatenate((train_x, np.mat(train_y)), axis=1)
        # train_xy = train_xy.astype(float)
        # train_xn, train_xp = Matrix_division(train_xy)
        #
        # showDataSet1(i, train_xp, train_xn)

        # 训练svdd
        # fit the SVDD model
        # exec(f"svdd{i} = BaseSVDD(C=0.6, gamma=10, kernel='rbf', display='on',degree=7)")
        # eval(f'svdd{i}').fit(np.array( prepare_list['subspace_matrix' + str(i)]), np.array(y))
        ...
        # print("Score",eval(f'adboost_{i}').score(prepare_list['subspace_matrix' + str(i)],y))
        # plot the distance curve
        # radius = eval(f'svdd{i}').radius
        # distance = eval(f'svdd{i}').get_distance(prepare_list['subspace_matrix' + str(i)])
    ...
    # print("sub_density_center",np.shape(sub_density_center))
    ...
    for k in range(np.shape(testArr)[0]):
        ...
        for er in range(int(np.shape(sub_density_center)[0]/3)):
            all = all +dis_a[er,1]


        predict_all = 0
        # for re in range(int(np.shape(sub_density_center)[0]/2)):
        for re in range(int(np.shape(sub_density_center)[0]/3)):
            # print("re",re)
            # print(dis_a)
            # print(dis_a[re, 0])
