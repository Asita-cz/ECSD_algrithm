...
def load_data(path,dim):
    ...
    dataArr, testArr, LabelArr, testLabelArr = train_test_split(dataset, Y, test_size=0.2, random_state=None, stratify=Y)
...
clf = SelfPacedEnsembleClassifier(base_estimator=DecisionTreeClassifier(),n_estimators=10).fit(dataArr, LabelArr)
...
print ("负例个数：", np.sum (np.mat (LabelArr).T == 0))
print ("测试集集个数：", len (testArr))
print ("正例个数：", np.sum (np.mat (testLabelArr).T == 1))
print ("负例个数：", np.sum (np.mat (testLabelArr).T == 0))

print ("TP: ", errArr[(predict == 1) & (predict == np.mat (testLabelArr).T)].sum ())
print ("FN: ", errArr[(predict == 0) & (predict != np.mat (testLabelArr).T)].sum ())
print ("TN: ", errArr[(predict == 0) & (predict == np.mat (testLabelArr).T)].sum ())
print ("FP: ", errArr[(predict == 1) & (predict != np.mat (testLabelArr).T)].sum ())
FN = errArr[(predict == 0) & (np.mat (testLabelArr) == 1).T].sum ()
FP = errArr[(predict == 1) & (np.mat (testLabelArr) == 0).T].sum ()
...
Specificity_Test = errArr[(predict ==0) & (np.mat (testLabelArr).T == 0)].sum () / (
        errArr[(predict == 0) & (np.mat (testLabelArr).T == 0)].sum () + FP)
