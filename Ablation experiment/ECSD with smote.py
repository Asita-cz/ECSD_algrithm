...
if __name__ == '__main__':
    ...
    dataArr, testArr, LabelArr, testLabelArr = load_data(path, dim,testsize=test_size)
    sampling_strategy = 1
    # 执行 SMOTE 过采样
    smote = SMOTE(sampling_strategy=sampling_strategy, random_state=42)
    dataArr, LabelArr = smote.fit_resample(dataArr, LabelArr)
    ...
    for i in range(len(leaf)):
        ...
        exec(
            f"GBDT{i} = GradientBoostingClassifier(learning_rate=0.8, n_estimators=2000, min_samples_leaf=20, max_features='sqrt', subsample=0.8, random_state=0)")
